...
class DataObject():
    ...
    def analyse_drift(self, chunk_size=250, freq_range=None, plot_waterfall=True):
        ...
        # Create a normalized time scale for chunk processing
        # This converts the time from [-2500, 2500] to [0, 5000] for example to combat loading issue
        normalised_time = time_array - start_time
        ...
        mode1_range = (100.0, 101.0)  # Adjust based on your data
        mode2_range = (98.5, 99.5)    # Adjust based on your data
        ...
        for chunk_idx in range(num_chunks):
            # Calculate time boundaries chunk by chunk using the normalized time scale
            normalized_chunk_start = chunk_idx * chunk_size
            normalized_chunk_end = min(normalized_chunk_start + chunk_size, total_duration)
        
            # Convert back to original time scale for data access
            chunk_start_time = start_time + normalized_chunk_start
            chunk_end_time = start_time + normalized_chunk_end
        
            # Calculate center time in original time scale
            ...
            freqs, psd = calc_PSD(chunk_voltage, self.SampleFreq, NPerSegment=1000000, window="hann")
...
def create_combined_stability_plots(mode1_times, mode1_freqs, mode1_amps, 
                                   ...
                                   file_paths):
    """
    Create plots showing the combined stability across all processed files.
    ...
    if mode1_freqs:
        _plt.plot(mode1_times, mode1_freqs, 'o-', color='blue', 
                 label=f"Mode 1: {_np.mean(mode1_freqs):.2f} Hz", alpha=0.7)
...
if __name__ == "__main__":
    ...
    base_dir = "C:/Users/migsh/Desktop/moon325/"
    ...
    # Analyse all files sequentially
    results = analyse_multiple_files(
        file_paths,
        chunk_size=250,          # seconds per chunk
        freq_range=[98, 102],    # frequency range to analyze
